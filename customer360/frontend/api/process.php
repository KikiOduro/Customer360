 ...
 *
 * The browser calls this PHP file, and this file calls FastAPI with the stored JWT.
 * That keeps the private backend URL out of the user's browser.
 */
...
// Route one compact PHP endpoint into the specific FastAPI job operation requested.
switch ($action) {
    case 'status':
        handleStatus($jobId);
        break;
...
function requireToken(): string {
    // Every job operation needs the JWT that was saved during login.
...
function jsonAuthExpired(string $message = 'Your session has expired. Please sign in again.'): void {
    // If FastAPI rejects the token, remove stale PHP session state as well.
...
function handleStatus($jobId) {
    // Status drives processing.php progress, stage labels, and the View Results button.
    if (!$jobId) {
...
function handleResults($jobId) {
    // Results are cached into the PHP session for pages that need quick reuse.
    if (!$jobId) {
        jsonResponse(['error' => 'No job ID provided'], 400);
    }
...
function handleReport($jobId) {
    // Stream the backend-generated PDF through PHP so auth stays consistent.
    if (!$jobId) {
        jsonResponse(['error' => 'No job ID provided'], 400);
    }
...
function handleCustomerCsv($jobId) {
    // Stream the segmented customer export through PHP for the same reason as PDFs.
    if (!$jobId) {
        jsonResponse(['error' => 'No job ID provided'], 400);
    }
...
function streamBackendAsset(string $url, string $token, string $acceptHeader, string $fallbackContentType, string $fallbackContentDisposition) {
    // Shared streaming helper for PDFs, CSVs, and chart images generated by FastAPI.
