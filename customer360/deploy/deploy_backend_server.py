...
def ensure_venv(backend_dir: Path, python_bin: str) -> Path:
    ...
    if not venv_python.exists() and legacy_venv_python.exists():
        print(f"WARNING: {venv_python} is missing, using legacy interpreter {legacy_venv_python}")
        run_command([str(legacy_venv_python), "-m", "ensurepip", "--upgrade"], cwd=backend_dir, check=False)
        run_command([str(legacy_venv_python), "-m", "pip", "install", "--upgrade", "pip"], cwd=backend_dir)
        run_command([str(legacy_venv_python), "-m", "pip", "install", "-r", "requirements.txt"], cwd=backend_dir)
        return legacy_venv_python

    if not venv_python.exists():
        run_command([python_bin, "-m", "venv", str(venv_dir)], cwd=backend_dir)

    run_command([str(venv_python), "-m", "ensurepip", "--upgrade"], cwd=backend_dir, check=False)
    run_command([str(venv_python), "-m", "pip", "install", "--upgrade", "pip"], cwd=backend_dir)
    run_command([str(venv_python), "-m", "pip", "install", "-r", "requirements.txt"], cwd=backend_dir)
    return venv_python
