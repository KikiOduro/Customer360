...
- Segment cards on `analytics.php` now use Material Symbols icons such as `kid_star`, `favorite`, `trending_up`, and `warning` instead of emoji glyphs. Fresh segmented customer CSV exports no longer include the `segment_emoji` column because the emoji field was presentation-only and not needed in exported business data.
