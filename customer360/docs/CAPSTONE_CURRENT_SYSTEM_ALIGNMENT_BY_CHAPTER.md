...
- Segment cards on `analytics.php` now use Material Symbols icons such as `kid_star`, `favorite`, `trending_up`, and `warning` instead of emoji glyphs. Segmented customer CSV exports no longer include presentation-only icon columns such as `segment_emoji` or `segment_icon`; the backend now strips those fields from old saved CSV files at download time as well.
